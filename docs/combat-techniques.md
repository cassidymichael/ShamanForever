...
- **Open a picker, in combat too:** a `SecureHandlerBaseTemplate` header holds a frame reference to each picker (a protected frame) and two snippets: open one picker and hide the others, or close one. Each button's click is wrapped with `SecureHandlerWrapScript` and runs one of them: the arrow tab toggles its picker, Alt+left-click on a slot opens it (the pre-snippet returns `false`, so the slot doesn't cast), and the picker's buttons use `multispell` (spell `0` is "No totem") and then close it in the post-snippet. An invisible screen-wide button inside the picker closes it on any other click, and the arrow tabs sit above it, so another slot's arrow switches pickers in one click.

  Before build 70009 snippets didn't run on Forever, and the pickers drove Blizzard's `SecureStateDriverManager` instead: the built-in `attribute` action set its `setframe` and `setstate` attributes, from macros that `/click`ed named helper buttons. That works too, but `setframe` is shared by every state driver in the game.
- **Which totem is down:** the slot's icon is secret in combat, but `SetTexture` accepts the secret value and draws it. Timers come from `GetTotemDuration`, and the expiring warning and "not your pick" badge follow the same rules as above (curves into `SetAlpha`; our own casts for which totem it is, by spell ID, ranks matched by the game's own name for the spell).
- **Out of range:** no addon can measure the distance to a totem, and in combat every aura is secret (`GetPlayerAuraBySpellID` returns nothing for all of them). Blizzard's `CustomAuraContainer` can still show one: per slot, an aura slot filtered to `HELPFUL|PLAYER` (buffs you cast) and the element's totem buffs draws the green part while you have the buff. Under it our red part shows while one of your buff totems is down (a curve on the slot's duration, and which totem from your own cast). Blizzard's part draws an opaque slice of the buff's icon under its colour, so the red never shows through a see-through green. A buff lingers a few seconds after you leave range; another shaman's same totem can replace your buff.
