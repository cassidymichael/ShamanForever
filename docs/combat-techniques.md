...
- **Open a picker, in combat too:** Blizzard's `SecureStateDriverManager` takes its instructions as attributes (`setframe`, then `setstate` = `"state-visibility show"`), and the built-in `attribute` action can set them. `setframe` is shared by every state driver in the game, so a release never trusts what an earlier press left there: a button with `pressAndHoldAction` runs a macro on release that `/click`s named helper buttons, one pointing `setframe` at its own picker and one showing or hiding it. The arrow tab (and Alt+click on a slot) opens its picker that way, closing the others in the same macro; the picker's buttons use `multispell` (spell `0` is "No totem") on press and hide the picker on release. An invisible screen-wide button inside the picker closes it on any other click, and the arrow tabs sit above it, so another slot's arrow switches pickers in one click.
